...
<table width="100%" class="table_form ">
    ...
    <tr>
        ...
        <td id="bindcat">
            <span>
               <?php echo ' <b>'.catpos($cat[0]['catid'],'->').'</b>'; ?>(不可修改)
            </span>
            <input type="hidden" value="<?php echo $cat[0]['catid']; ?>" name="data[catid]">
         </td>
    </tr>
    <tr>
        <th>广告类型：</th>
...
function post(catid)
{
    var url  =  $('#url').val()+'&catid='+catid;
        ...
        {   
            var obj =   eval('('+data+')');
            var select = $('<select class="cat newcat" name="data[catids][]"><option value="0">全部</option></select>');
