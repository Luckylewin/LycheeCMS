...
class ContentController extends Common {
	...
	public function listAction() {
        ...
        $catdir && empty($catid) && $catid = $this->cats_dir[$catdir];
	    ...
	    if ($cat['typeid'] == 1) {
            ...
            $tpl = $cat['child'] == 1 && is_file(FCPATH.'views/'.$this->site['SITE_THEME'].'/'.$cat['categorytpl']) ? $cat['categorytpl'] : $cat['listtpl'];
	        ...
	        $cat['content'] = relatedlink($cat['content']);
            $modelid = $cat['modelid'];
            empty($modelid) && $this->msg(lang('for-0'));

            $model = $this->get_model('form');
            $model = $model[$modelid];
            empty($model) && $this->msg(lang('for-1', array('1'=>$modelid)));
            ...
            !$cat['showtpl'] && $cat['showtpl'] = 'post_form.html';
			$this->view->assign($cat);
			$this->view->assign(listSeo($cat, $page));
	        $this->view->display(substr($cat['showtpl'], 0, -5));
	    } else {
	...
	public function showAction() {
	    ...
	    if (empty($data) || $data['status'] == 0) {	//判断数据是否存在或文档状态是否通过
		    ...
		    header('HTTP/1.1 404 Not Found');
			$this->msg(lang('con-0', array('1' => $catid)));
		}
		...
		if ($page <= 1 && strpos(fn_now_url(), $data['url']) === false) {
			// 301 定向地址
