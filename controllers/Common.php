...
class Common extends CI_Controller
{
    ...
    public function __construct()
    {
		...
		if ($uip && $ips && is_array($ips)) {
			...
			foreach ($ips as $ip => $t) {
                ...
                preg_match('/^' . str_replace(array('*', '.'), array('[0-9]+', '\.'), $ip) . '$/', $uip)
                && $this->adminMsg(lang('a-aip-6'));
    ...
    protected function is_ie()
    {
		...
		if (in_array($b, array('Firefox', 'Opera', 'Chimera', 'Safari', 'Chrome'))) {
			return false;
		} elseif ($b == 'Internet Explorer' && $v >=9) {
			return false;
		}
	...
	*/
    ...
    private function _init() {
        ...
        $mb = is_file(CONFIG_DIR . 'mobile.ini.php') ? array_flip(require CONFIG_DIR . 'mobile.ini.php') : array();
        ...
        foreach ($site as $url => $sid) {
            ...
            if (file_exists($f) && is_file($f)) {
                ...
                $info[$sid]['DOMAIN'] = $url;
				if ($info[$sid]['SITE_JS']) {
					$tmp = string2array($info[$sid]['SITE_JS']);
					$info[$sid]['SITE_JS'] = html_entity_decode(str_replace('{|}', "'", $tmp['value']));
				}
				$info[$sid]['SITE_BOTTOM_INFO'] = @html_entity_decode($info[$sid]['SITE_BOTTOM_INFO']);
            } else {
     ...
     * 保存非法字符攻击日志
     */
    public static function save_attack_log($type, $val) {
