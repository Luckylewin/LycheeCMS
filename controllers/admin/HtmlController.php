...
class HtmlController extends Admin {
    ...
    public function categoryAction() {
        if ($this->isPostForm()) {
            ...
            if ($this->site['SITE_EXTEND_ID']) {
                $this->htmlMsg(lang('a-sit-29', array('1' => $sites[$this->site['SITE_EXTEND_ID']]['SITE_NAME'])));
            }
            foreach ($sites as $sid => $t) {
                if ($t['SITE_EXTEND_ID'] && $t['SITE_EXTEND_ID'] == $this->siteid) {
                    $this->htmlMsg(lang('a-sit-30', array('1' => $t['SITE_NAME'])));
                }
            }
            $this->htmlMsg(lang('a-cat-101'), url('admin/html/category', array('submit' => 1, 'catid' => $catid, 'isall' => $isall)), 0, 1, 2);
        }
        ...
        if ($submit) {
            ...
            if ($catid && $isall == 0) {
                ...
                if (isset($cats[$key]) && $this->cats[$cats[$key]]) {
                    ...
                    $this->cache->delete('html_cats');
                    $this->htmlMsg(lang('a-con-107') . '(' . $count . ')', '', 0, 1, 1);
                }
            } else {
                if (empty($catid)) {
                    $cats = $this->cats;
    ...
    public function showAction() {
        if ($this->isPostForm()) {
            $this->cache->delete('html_cats');
            $this->htmlMsg(lang('a-cat-101'), url('admin/html/show', array('submit' => 1, 'catid' => $this->post('catid'), 'totime' => $this->post('totime'))), 0, 1, 2);
        }
        $submit = (int)$this->get('submit');
        if ($submit) {
            $page = $this->get('page') ? $this->get('page') : 1;
    ...
    public function formAction() {
        if ($this->isPostForm()) {
            $this->htmlMsg(lang('a-cat-101'), url('admin/html/form', array('submit' => 1, 'mid' => $this->post('mid'))), 0, 1, 2);
        }
        $submit	= (int)$this->get('submit');
        ...
        if ($submit) {
            ...
            if ($page >= $totalpage) {
                ...
                $this->htmlMsg('【' . $form[$mid]['modelname'] . '】(' . $page.'/' . $totalpage .')', url('admin/html/form',array('page' => $nextpage, 'mid' => $mid, 'submit' => 1,'count' => $count)), 3, 1, 2);
            }
        } else {
    ...
    public function indexcAction() {
        ...
        $this->htmlMsg(lang('a-con-107') . '(' . formatFileSize($size) . ')', '', 3, 1, 1);
    }
    ...
    public function clearAction() {
        $submit	= (int)$this->get('submit');
        if (empty($submit)) {
            $this->htmlMsg(lang('a-con-108'), url('admin/html/clear', array('submit' => 1)), 3, 1, 2);
        }
        @unlink('index.html');
        $htmlfiles	= $this->cache->get('html_files');
        if (empty($htmlfiles)) {
            $this->htmlMsg(lang('a-con-109'), '', 3, 1, 1);
        }
        ...
        $this->htmlMsg(lang('a-con-110', array('1' => $d, '2' => $f)), '', 3, 1, 1);
    }
    ...
    private function toCategory($catid, $page, $isall, $key, $filecount) {
        ...
        $this->htmlMsg('【' . $cat['catname'] . '】(' . $page . '/' . $totalpage . ')', url('admin/html/category', array('page' => $nextpage, 'catid' => $nextcatid, 'isall' => $isall, 'key' => $key, 'filecount' => $filecount, 'submit' => 1)), 0, 1, 2);
    }
    ...
    private function toContent($cats, $page, $filecount, $totime = 0) {
        ...
        if (empty($data)) {
            $this->cache->delete('html_cats');
            $this->htmlMsg(lang('a-con-107') . '(' . $filecount . ')', '', 0, 1, 1);
        }
        foreach ($data as $t) {
        ...
        $this->htmlMsg(lang('a-con-111') . " ($page/$totalpage)", url('admin/html/show', array('page' => $page+1, 'submit' => 1, 'filecount' => $filecount, 'totime' => $totime)), 0, 1, 2);
    }
