	        ...
	        foreach ($_POST as $var=>$value) {
	            if (strpos($var, 'del_') !== false) {
					...
					if ($data['contentid']) { //判断该推荐信息是否来至文档内容
					    $cdata = $this->content->get_extend_data((int)$data['contentid']);
						if ($cdata['position']) {
							...
							$pn = @implode(',', $pn);
							$cdata['position'] = $pn; 
							$this->content->set_extend_data((int)$data['contentid'], $cdata); //删除文档中的推荐位信息
						}
					}
	    ...
	    foreach ($position as $t) {
			...
			if ($data[$posid]) {
			    foreach ($data[$posid] as $id=>$c) {
				    if ($c['contentid']) {
					    $row = $this->content->find((int)$c['contentid']);
